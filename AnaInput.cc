...
AnaInput::AnaInput( string datacardInput ) {
  ...
  datacardpath = getenv("PWD") ;
  datacardpath +="/" ;
  datacardfile = datacardpath ;
  datacardfile += datacardInput ;
}
...
void AnaInput::GetParameters(string paraName, int* thePara, string cfgFile ){
     ...
     if ( cfgFile.size() < 1 ) cfgFile = datacardfile ;

     fstream paraFile( cfgFile.c_str() );
     if ( !paraFile.is_open() ) {
         string fileWithPath = datacardpath ;
         fileWithPath += cfgFile ;
         paraFile.open( fileWithPath.c_str() ) ;
